...
void read_start() {
    ...
    for (int y = 0; y < height_map.size(); y++) {
        if ((x = height_map[y].find('E')) == (std::string::npos))
            continue;
...
bool valid_move(std::array<int,3> current_coords, std::array<int,3> next_coords) {
    ...
    current_pos = (current_pos == 'E') ? 'z' : current_pos;
    next_pos = (next_pos == 'S') ? 'a' : next_pos;
...
void propagate_first_search(char target) {
    ...
    for (std::array<int,2> offset : possible_moves) {
        ...
        if (height_map[offset_coords[1]][offset_coords[0]] == target) {
...
int main() {
    ...
    length_found = 0;
    while (length_found == 0 && search_threads.size() > 0) propagate_first_search('S');
    std::cout << "Part 1: " << length_found << std::endl;

    cleanse_search();
    length_found = 0;
    while (length_found == 0 && search_threads.size() > 0) propagate_first_search('a');
    std::cout << "Part 2: " << length_found << std::endl;
